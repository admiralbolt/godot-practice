...
func _ready() -> void:
  ...
  self.is_open_data.data_loaded.connect(_set_chest_state)
  _set_chest_state()
...
func _set_chest_state() -> void:
  self.is_open = self.is_open_data.value
  self.animation_player.play("opened" if self.is_open else "closed")
